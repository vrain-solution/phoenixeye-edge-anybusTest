...
static void appl_GetTim5Cnt( const struct AD_AdiEntry* psAdiEntry, UINT8 bNumElements, UINT8 bStartIndex )
{
  ...
  */
  ...
  BSP_LCD_DisplayStringAt(SPACE(8), LINE(9), (unsigned char *)text, LEFT_MODE);
  #endif
  appl_iRdCnt++;
}
...
static void appl_Set4Leds( const struct AD_AdiEntry* psAdiEntry, UINT8 bNumElements, UINT8 bStartIndex )
{
  ...
  if(bLedStatusLast != appl_b4Leds)
  {
    ...
    */
    ...
    BSP_LCD_DisplayStringAt(SPACE(12), LINE(6), (unsigned char *)text, LEFT_MODE);
    #endif
  }
  appl_iWrCnt++;
}
...
static void appl_Set8Leds8IOs( const struct AD_AdiEntry* psAdiEntry, UINT8 bNumElements, UINT8 bStartIndex )
{
  ...
  if((iLastState != appl_i8Leds8IOs))
  {
    ...
    ** Logically invert "just" the LEDs to be consistent */
    ...
    #ifdef __USE_LCD
    BSP_LCD_SetTextColor(LCD_COLOR_RED);
    snprintf(text, sizeof(text), "0x%02X", (UINT8)((appl_i8Leds8IOs&0x00FF)));
    BSP_LCD_DisplayStringAt(SPACE(12), LINE(7), (unsigned char *)text, LEFT_MODE);
    snprintf(text, sizeof(text), "0x%02X", (UINT8)((appl_i8Leds8IOs)>>8));
    BSP_LCD_DisplayStringAt(SPACE(12), LINE(8), (unsigned char *)text, LEFT_MODE);
    #endif
  }
  appl_iWrCnt++;
}
...
static void appl_GetKey( const struct AD_AdiEntry* psAdiEntry, UINT8 bNumElements, UINT8 bStartIndex )
{
  ...
  else
  {
    ...
    BSP_LCD_DisplayStringAt(SPACE(12), LINE(5), (unsigned char *)"TRUE", LEFT_MODE);
    BSP_LCD_SetBackColor(LCD_COLOR_WHITE);
    BSP_LCD_SetTextColor(LCD_COLOR_RED);
    BSP_LCD_DisplayStringAt(SPACE(16), LINE(5), (unsigned char *)" ", LEFT_MODE);
    #endif
  }
  appl_iRdCnt++;
}
...
static void appl_GetPot( const struct AD_AdiEntry* psAdiEntry, UINT8 bNumElements, UINT8 bStartIndex )
{
  ...
  ** number whose scale ranges from 0.0f to 3.3f*/
  ...
  iDispVal = (SPACE(10)*(iRawAdcVal>>1))/2048;
  BSP_LCD_SetBackColor(LCD_COLOR_RED);
  BSP_LCD_SetTextColor(LCD_COLOR_RED);
  BSP_LCD_FillRect(SPACE(8), LINE(4)+2, iDispVal, LINE(1)-4);

  BSP_LCD_SetBackColor(LCD_COLOR_LIGHTGRAY);
  BSP_LCD_SetTextColor(LCD_COLOR_LIGHTGRAY);
  BSP_LCD_FillRect(SPACE(8)+iDispVal, LINE(4)+2, SPACE(10)-iDispVal, LINE(1)-4);
  BSP_LCD_SetBackColor(LCD_COLOR_WHITE);
  #endif
  appl_iRdCnt++;
}
